...
String _email(String first, String last, int idx) => '${first.toLowerCase()}.${last.toLowerCase()}$idx@example.com';

String _phone() => '555-${1000 + _rand.nextInt(9000)}';

List<String> _majors = [
  'Computer Science', 'Business', 'Economics', 'Electrical Engineering', 'Mechanical Engineering', 'Biology', 'Mathematics', 'Design'
];

List<String> _firstNames = [
  'Alice','Bob','Carol','David','Eve','Frank','Grace','Hank','Ivy','Jack','Karen','Leo','Mona','Nate','Olivia','Paul','Quinn','Rita'
];

List<String> _lastNames = [
  'Anderson','Brown','Clark','Diaz','Evans','Foster','Garcia','Harris','Ivanov','Johnson','Khan','Lee','Martinez','Nguyen','Olsen'
];
...
Future<void> main() async {
  ...
  final adminCheck = await connection.execute(Sql.named('SELECT id FROM users WHERE email = @email'), parameters: {'email': adminEmail});
  int adminId;
  if (adminCheck.isNotEmpty) {
    ...
    final adminRes = await connection.execute(
      Sql.named('INSERT INTO users (email, password, first_name, last_name, role, status, major, graduation_year, phone_number, profile_image_url) VALUES (@email, @pass, @first, @last, @role, @status, @major, @grad, @phone, @profile) RETURNING id'),
      parameters: {
        'email': adminEmail,
        'pass': adminHash,
        'first': 'Site',
  ...
  for (var i = 0; i < 20; i++) {
    ...
    final profileUrl = useProfile ? 'http://localhost:8080/uploads/profiles/profile_${(i % 6) + 1}.jpg' : null;

    // skip insert if email exists
    final exists = await connection.execute(Sql.named('SELECT id FROM users WHERE email = @email'), parameters: {'email': email});
  ...
  final alumniIds = inserted.where((e) => e['role'] == 'alumni').map((e) => e['id'] as int).toList();
  ...
  for (var i = 0; i < 12; i++) {
    ...
    await connection.execute(
      Sql.named('INSERT INTO posts (author_id, title, content, type, image_url) VALUES (@a, @t, @c, @ty, @img)'),
      parameters: {
        'a': author,
        't': '${_pick(['Campus Reunion', 'Mentorship Launch', 'Alumni News', 'Career Tips', 'Volunteer Call'])} #${i + 1}',
  ...
  final companies = ['Acme Corp', 'Globex', 'Innotech', 'Stellar Labs', 'Brightside'];
  final jobs = [
  ...
  for (var i = 0; i < 6; i++) {
    ...
    await connection.execute(
      Sql.named('INSERT INTO jobs (company_name, job_title, description, location, salary_range, contact_email, posted_by_id) VALUES (@c, @j, @d, @l, @s, @e, @p)'),
      parameters: {
        'c': comp,
        'j': job['title'],
  ...
  for (var i = 0; i < 10; i++) {
    ...
    final action = _pick(['LOGIN', 'UPDATE_PROFILE', 'CREATE_POST', 'APPLY_JOB']);
    await connection.execute(Sql.named('INSERT INTO activity_logs (user_id, action, details) VALUES (@u, @a, @d)'), parameters: {'u': user, 'a': action, 'd': 'Seeded action $action'});
  }
